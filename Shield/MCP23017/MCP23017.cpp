...
/*void MCP23017::write_mask(unsigned char data, unsigned char mask) {
    shadow_GPIO = (shadow_GPIO & ~mask) | data;
    writeRegister(GPIO, (unsigned short)shadow_GPIO);
}
*/
...
char MCP23017::readRegister8(int regAddress) {
    char buffer[2];

    buffer[0] = regAddress;
    _i2c.write(MCP23017_i2cAddress, buffer, 1, true);
    _i2c.read(MCP23017_i2cAddress, buffer, 1);
 ...
 * digitalPinToBitMask
 */
unsigned short MCP23017::digitalPinToBitMask(int pin) {
    unsigned short mask = 2^pin;
    return mask;
}